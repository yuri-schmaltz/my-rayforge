...
if [[ "$1" == "pip" || -z "$1" ]]; then
    ...
    $PYTHON_BIN_PATH -m pip install --no-cache-dir pyserial_asyncio_fast==0.16 --break-system-packages
    $PYTHON_BIN_PATH -m pip install --no-cache-dir ezdxf==1.3.5 pypdf==5.3.1 trimesh==4.6.8 --break-system-packages
