...
parser.add_argument(
    "--requirements-file", "-r", help="Specify requirements.txt file"
)
parser.add_argument(
    "--build-only",
    action="store_const",
...
parser.add_argument(
    "--checker-data",
    action="store_true",
    help="Include x-checker-data in output for the "
    '"Flatpak External Data Checker"',
)
parser.add_argument("--output", "-o", help="Specify output file name")
parser.add_argument(
    "--runtime",
    help="Specify a flatpak to run pip inside of a sandbox, "
...
parser.add_argument(
    "--ignore-pkg",
    nargs="*",
    help="Ignore a package when generating the manifest. "
...
def get_package_name(filename: str) -> str:
    if filename.endswith(("bz2", "gz", "xz", "zip")):
        ...
        # e.g. PyQt5-5.15.0-5.15.0-cp35.cp36.cp37.cp38-abi3
        ...
        raise Exception(
            "Downloaded filename: {} does not end with "
            "bz2, gz, xz, zip, or whl".format(filename)
        )
    ...
    for line in fin:
        ...
        while line.endswith("\\"):
            try:
                ...
                exit(
                    "Requirements have a wrong number of "
...
for i in packages:
    if i["name"].lower().startswith("pyqt"):
        ...
        print(
            "However, there is a BaseApp for PyQt available, "
            "that you should use"
        )
        print(
            "Visit https://github.com/flathub/"
            "com.riverbankcomputing.PyQt.BaseApp for more information"
        )
        sys.exit(0)
    ...
    for filename in os.listdir(tempdir):
        ...
        if name in vcs_packages:
            ...
            if opts.checker_data:
                source["x-checker-data"] = {  # type: ignore
                    "type": "pypi",
                    "name": name,
                }
                if url.endswith(".whl"):
...
for package in packages:
    if package.name is None:
        print(
            "Warning: skipping invalid requirement specification {} "
            "because it is missing a name".format(package.line),
            file=sys.stderr,
        )
        print(
    ...
    for dependency in dependencies:
        ...
        if casefolded in sources and sources[casefolded].get("pypi") is True:
            ...
            dependency in sources and sources[dependency].get("pypi") is False
        ):
    ...
    if opts.cleanup == "all":
        ...
        module["cleanup"] = ["/bin", "/share/man/man1"]  # type: ignore
...
if len(unresolved_dependencies_errors) != 0:
    ...
    workaround = """Example how to handle wheels which only
    support specific architectures:
    - type: file
      url: https://files.pythonhosted.org/packages/79/ae/...whl
      sha256: 7ab55401287bfec946ced39700c053796e7cc0e3acbef09993a9ad2adba6ca6e
      only-arches:
      - aarch64
    - type: file
      url: https://files.pythonhosted.org/packages/3a/d0/...whl
      sha256: 666dbfb6ec68962c033a450943ded891bed2d54e6755e35e5835d63f4f6931d5
      only-arches:
      - x86_64
    """
    raise Exception(
        "Not all dependencies can be determined. "
