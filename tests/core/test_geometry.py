...
def test_get_winding_order(ccw_square_geometry, cw_square_geometry):
    # Test CCW
    assert ccw_square_geometry.get_winding_order(1) == "ccw"
    assert ccw_square_geometry.get_winding_order(3) == "ccw"

    # Test CW
    assert cw_square_geometry.get_winding_order(1) == "cw"
    assert cw_square_geometry.get_winding_order(3) == "cw"

    # Test open path
    open_geo = Geometry()
    open_geo.move_to(0, 0)
    open_geo.line_to(10, 10)
    assert open_geo.get_winding_order(1) == "unknown"
...
def test_get_outward_normal_at(ccw_square_geometry, cw_square_geometry):
    ...
    normal = ccw_square_geometry.get_outward_normal_at(1, 0.5)
    assert normal is not None
    assert normal == pytest.approx((0, -1))
    # Right edge, tangent (0,1) -> outward normal (1,0)
    normal = ccw_square_geometry.get_outward_normal_at(2, 0.5)
    assert normal is not None
    assert normal == pytest.approx((1, 0))
    # Top edge, tangent (-1,0) -> outward normal (0,1)
    normal = ccw_square_geometry.get_outward_normal_at(3, 0.5)
    assert normal is not None
    assert normal == pytest.approx((0, 1))
    # Left edge, tangent (0,-1) -> outward normal (-1,0)
    normal = ccw_square_geometry.get_outward_normal_at(4, 0.5)
    assert normal is not None
    assert normal == pytest.approx((-1, 0))

    # Test CW square
    # Left edge, tangent (0,1) -> outward normal (-1,0)
    normal = cw_square_geometry.get_outward_normal_at(1, 0.5)
    assert normal is not None
    assert normal == pytest.approx((-1, 0))
    # Bottom edge, tangent (-1,0) -> outward normal (0,-1)
