...
class TestMakeTransparentExceptColor:
    """Tests for `make_transparent_except_color`."""

    def test_keeps_target_color_opaque(self):
        surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 2, 2)
        data = surface.get_data()
        arr = np.frombuffer(data, dtype=np.uint8).reshape((2, 2, 4))
        arr[:, :, 0] = 100
        ...
        util.make_transparent_except_color(surface, 200, 150, 100)

        data = surface.get_data()
        arr = np.frombuffer(data, dtype=np.uint8).reshape((2, 2, 4))
        assert arr[0, 0, 3] == 255

    def test_makes_other_colors_transparent(self):
        surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 2, 2)
        data = surface.get_data()
        arr = np.frombuffer(data, dtype=np.uint8).reshape((2, 2, 4))
        arr[:, :, 0] = 50
    ...
    def test_raises_for_non_argb32(self):
        ...
        with pytest.raises(
            ...
            util.make_transparent_except_color(surface, 255, 0, 0)
    ...
    def test_mixed_colors(self):
        ...
        util.make_transparent_except_color(surface, 0, 255, 0)
