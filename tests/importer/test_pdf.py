...
import io
import cairo
from rayforge.importer.pdf import PdfImporter
...
@pytest.fixture
def basic_pdf_importer():
    return PdfImporter(create_basic_pdf())


@pytest.fixture
def large_pdf_importer():
    ...
    surface.finish()
    buf.seek(0)
    return PdfImporter(buf.getvalue())


class TestPDFImporter:
    def test_get_natural_size(self, basic_pdf_importer):
        """Test natural size in millimeters for a 100pt x 50pt PDF."""
        width_mm, height_mm = basic_pdf_importer.get_natural_size()
    ...
    def test_get_aspect_ratio(self, basic_pdf_importer):
        """Test aspect ratio calculation (width/height)."""
        ratio = basic_pdf_importer.get_aspect_ratio()
        assert ratio == pytest.approx(2.0, abs=0.01)  # 100pt / 50pt = 2.0

    def test_render_to_pixels(self, basic_pdf_importer):
        """
        Test rendering to a Cairo surface at a specified size.
        """
        surface = basic_pdf_importer.render_to_pixels(width=200, height=100)
        assert isinstance(surface, cairo.ImageSurface)
        assert surface.get_width() == 200
        assert surface.get_height() == 100

    def test_render_chunk(self, large_pdf_importer):
        """Test chunk rendering without overlap."""
        chunks = list(
            large_pdf_importer.render_chunk(
                width_px=1000,
                height_px=500,
                max_chunk_width=300,
    ...
    def test_render_chunk_overlap(self, basic_pdf_importer):
        """Test chunk rendering with overlap."""
        chunks = list(
            basic_pdf_importer.render_chunk(
                width_px=100,
                height_px=50,
                max_chunk_width=40,
                max_chunk_height=30,
    ...
    def test_invalid_pdf_handling(self):
        ...
        importer = PdfImporter(invalid_pdf_data)
        ...
        assert importer.render_to_pixels(100, 100) is None

        # The chunk importer should be an empty generator.
        chunks = list(importer.render_chunk(100, 100, max_chunk_width=100))
        assert len(chunks) == 0
