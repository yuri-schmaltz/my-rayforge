...
def create_mock_package(name="pkg", version="1.0.0"):
    mock_pkg = MagicMock(spec=Package)
    # Configure the nested metadata structure
    mock_pkg.metadata = MagicMock()
...
def create_mock_remote_meta(
    name="pkg", version="1.1.0", url="http://a.b/c.git"
):
    ...
    mock_meta = MagicMock(spec=PackageMetadata)
    mock_meta.name = name
...
class TestUpdateCheck:
    async def test_check_for_updates_worker_no_updates(
        ...
        """
        Verify that no signal is sent when the package manager finds no
        updates.
        """
        # Arrange
        mock_context.package_mgr.check_for_updates.return_value = []
        ...
        with patch.object(
            ...
            # Assert
            mock_context.package_mgr.check_for_updates.assert_called_once()
            mock_send.assert_not_called()
            mock_ctx_task.set_message.assert_has_calls(
                [
                    call(_("Checking for package updates...")),
                    call(_("Packages are up to date.")),
                ]
            )
    ...
    async def test_check_for_updates_worker_finds_updates(
        ...
        """
        # Arrange
        updates_payload = [(create_mock_package(), create_mock_remote_meta())]
        mock_context.package_mgr.check_for_updates.return_value = (
            updates_payload
        )
        mock_ctx_task = MagicMock()
    ...
    async def test_check_for_updates_worker_handles_exception(
        ...
        """
        # Arrange
        mock_context.package_mgr.check_for_updates.side_effect = Exception(
            "Network error"
        )
        mock_ctx_task = MagicMock()
...
class TestUpdateInstallation:
    async def test_install_updates_worker_all_succeed(
        ...
        """
        Test the installation worker when all package updates succeed.
        """
        # Arrange
        updates_to_install = [
            (
                create_mock_package("pkg1"),
                create_mock_remote_meta("pkg1", "1.1.0"),
            ),
            (
                create_mock_package("pkg2"),
                create_mock_remote_meta("pkg2", "2.1.0"),
            ),
        ]
        # Mock install_package to return a dummy path on success
        mock_context.package_mgr.install_package.return_value = "/fake/path"
        mock_ctx_task = MagicMock()
        ...
        with patch.object(
            ...
            # Assert
            assert mock_context.package_mgr.install_package.call_count == 2
            mock_send.assert_called_once()
            # Check message of the notification
            sent_kwargs = mock_send.call_args.kwargs
            assert "message" in sent_kwargs
            assert "2 packages successfully updated" in sent_kwargs["message"]
            mock_ctx_task.set_message.assert_called_with(
                _("All package updates installed!")
            )
    ...
    async def test_install_updates_worker_some_fail(
        ...
        mock_context.package_mgr.install_package.side_effect = [
            "/fake/path",
            None,
        ]
        mock_ctx_task = MagicMock()
        ...
        with patch.object(
            ...
            sent_kwargs = mock_send.call_args.kwargs
            assert "message" in sent_kwargs
            assert "1 packages updated" in sent_kwargs["message"]
    ...
    async def test_install_updates_worker_all_fail(
        ...
        mock_context.package_mgr.install_package.return_value = None
        mock_ctx_task = MagicMock()
        ...
        with patch.object(
            ...
            sent_kwargs = mock_send.call_args.kwargs
            assert "message" in sent_kwargs
            assert "Failed to update package." in sent_kwargs["message"]
            mock_ctx_task.set_message.assert_called_with(
                _("Finished with {num_failed} errors.").format(num_failed=1)
            )
    ...
    async def test_install_updates_worker_no_updates(
        ...
        # Assert
        mock_context.package_mgr.install_package.assert_not_called()
