...
import numpy as np
import pytest
from rayforge.pipeline.producer.rasterize import rasterize_horizontally, rasterize_vertically, Rasterizer
from rayforge.core.ops import Ops, OpsSectionStartCommand, OpsSectionEndCommand, SectionType, MoveToCommand, LineToCommand
...
def test_rasterize_horizontally_white(white_surface):
    ...
    ops = rasterize_horizontally(white_surface, ymax=1.0, pixels_per_mm=(10, 10), raster_size_mm=0.1)
...
def test_rasterize_vertically_white(white_surface):
    ...
    ops = rasterize_vertically(white_surface, ymax=1.0, pixels_per_mm=(10, 10), raster_size_mm=0.1)
    assert len(ops.commands) == 0


def test_rasterize_horizontally_black(black_surface):
    """Tests horizontal rasterization on a black surface."""
    ops = rasterize_horizontally(black_surface, ymax=1.0, pixels_per_mm=(10, 10), raster_size_mm=0.1)
    assert len(ops.commands) > 0
...
def test_rasterize_vertically_black(black_surface):
    ...
    ops = rasterize_vertically(black_surface, ymax=1.0, pixels_per_mm=(10, 10), raster_size_mm=0.1)
    assert len(ops.commands) > 0
    # More specific assertions can be added here based on expected output
...
def test_rasterizer_run_wraps_ops_in_section_markers(white_surface):
    """Even with an empty result from a white surface, the output should be correctly wrapped."""
    laser = MagicMock()
    rasterizer = Rasterizer()
    mock_workpiece = MagicMock(uid="wp_123")
