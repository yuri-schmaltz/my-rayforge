...
def register_transformers(transformer_registry):
    """
    Called to register custom ops transformers.
    """
...
def register_layout_strategies(layout_registry):
    """
    Called to register custom layout strategies.
    """
...
def step_settings_loaded(dialog, step, producer):
    ...
    Addons can add custom widgets to the dialog.
    """
...
: Called to allow addons to register custom ops transformers for
  post-processing operations. Transformers can modify operations after they
...
: Called to allow addons to register window actions. Use the action_registry
  to register actions with optional menu and toolbar placement parameters.

**`register_layout_strategies`** (`layout_registry`)
: Called to allow addons to register custom layout strategies for the UI.
  Layout actions should be registered via `register_actions`.

**`step_settings_loaded`** (`dialog`, `step`, `producer`)
: Called when a step settings dialog is being populated. Addons can add
  custom widgets to the dialog based on the step's producer type.

**`transformer_settings_loaded`** (`dialog`, `step`, `transformer`)
: Called when post-processing settings are being populated. Addons can add
  custom widgets for their transformers.
...
- **`context.ai_provider_mgr`**: AI provider manager for AI-powered features.
