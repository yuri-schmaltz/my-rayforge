...
Rayforge 窗口底部的面板提供对激光切割机位置的手动控制、实时机器状态、G 代码查看和用于监控操作的日志视图。
...
1. **点动控制**：手动移动和定位
2. **机器状态**：实时位置和连接状态
3. **控制台标签**：带有语法高亮的交互式 G 代码终端
4. **G 代码查看器标签**：查看生成的 G 代码
5. **工作坐标系（WCS）**：快速 WCS 选择

![底部面板](/screenshots/control-panel.png)

## 访问底部面板

底部面板始终在主窗口底部可见。可以通过以下方式切换：
...
底部面板包含一个 G 代码查看器标签，用于检查生成的 G 代码：

### 功能

- **语法高亮**：G 代码命令、参数和注释以不同颜色显示
- **行高亮**：当前执行的行在模拟和作业执行期间高亮显示
- **自动刷新**：在操作更改时自动更新 G 代码内容
