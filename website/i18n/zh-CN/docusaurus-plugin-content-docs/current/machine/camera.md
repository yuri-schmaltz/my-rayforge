...
Rayforge 包含一个引导式校准向导，可以自动校正这种畸变。以下是它的工作方式：

1. **打印校准卡** — Rayforge 提供一个可打印的图案（标记网格），您将其放在激光床上
2. **按照向导操作** — 校准向导引导您从床的不同位置捕获卡片的几张图像
3. **应用校正** — Rayforge 从捕获的图像计算畸变模型，并使用它来矫正相机叠加

校准后，相机叠加将显示明显更准确的床上内容表示。这对于广角镜头、偏心安装的相机或需要严格对齐公差的工作特别有帮助。

:::note 何时校准
当您注意到相机叠加即使经过仔细对齐仍然与实际床面不太吻合时，校准最有用。如果当前对齐看起来不错，您可能不需要它。但如果感觉略有偏差 — 特别是在画面边缘 — 运行校准向导通常会有所帮助。
:::
