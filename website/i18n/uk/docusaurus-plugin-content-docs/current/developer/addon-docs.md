...
1. `rayforge_addon.yaml` (Метадані)
2. Точка входу Python (наприклад, `addon.py`)

**Макет директорії:**

```text
my-rayforge-addon/
├── rayforge_addon.yaml  <-- Обов'язковий маніфест
├── addon.py             <-- Точка входу (логіка)
├── assets/              <-- Опціональні ресурси
└── README.md
```

## 3. Маніфест (`rayforge_addon.yaml`)
...
# Унікальний ідентифікатор для вашого аддону
...
# Залежності (аддон та обмеження версій)
depends:
  - rayforge>=0.27.0,~0.27

# Python файл для завантаження (відносно папки аддону)
