...
class ActionManager:
    ...
    def update_action_states(self, *args, **kwargs):
        ...
        selected_items = self.win.surface.get_selected_top_level_items()

        # Update tabbing action state
        is_single_workpiece = len(selected_items) == 1 and isinstance(
            selected_items[0], WorkPiece
        )
        has_steps = False
        if is_single_workpiece:
            wp = cast(WorkPiece, selected_items[0])
            if wp.layer and wp.layer.workflow:
                has_steps = wp.layer.workflow.has_steps()

        if "add-tabs-equidistant" in self.actions:
            self.actions["add-tabs-equidistant"].set_enabled(
                is_single_workpiece and has_steps
            )
        if "add-tabs-cardinal" in self.actions:
            self.actions["add-tabs-cardinal"].set_enabled(
                is_single_workpiece and has_steps
            )
    ...
    def on_add_tabs_equidistant(self, action, param):
        ...
        if not (
            ...
            and isinstance(selected_items[0], WorkPiece)
        ):
            return

        workpiece = cast(WorkPiece, selected_items[0])

        # A workpiece's tabs are applied to the ops generated by a step.
        # We need to associate the tabs with a step. For now, we'll
        # arbitrarily use the first step in the workpiece's layer.
        if not (
            workpiece.layer
            and workpiece.layer.workflow
            and workpiece.layer.workflow.has_steps()
        ):
            # In a real app, we would show a user-facing error message here.
            return

        step = workpiece.layer.workflow.steps[0]

        # In a real implementation, a dialog would pop up here to ask for
        # count and width. For this example, we use fixed values.
        # We assume `self.editor.tab.add_tabs` is implemented as per plan 4.1.
    ...
    def on_add_tabs_cardinal(self, action, param):
        ...
        # In a real implementation, a dialog would pop up here to ask for
        # width. For this example, we use a fixed value.
