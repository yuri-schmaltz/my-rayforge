...
from ..pipeline.modifier import Modifier, MakeTransparent, ToGrayscale
...
from ..pipeline.transformer import OpsTransformer, Optimize, Smooth
...
class Step(ABC):
    ...
    def _on_task_event_received(
        ...
        workpiece = self.workflow.layer._get_workpiece_by_uid(workpiece_uid)
        if not workpiece:
            logger.warning(
    ...
    def get_ops(self, workpiece: "WorkPiece") -> Optional["Ops"]:
        ...
        if not self.workflow:
            logger.warning(
                f"Cannot get_ops for Step '{self.name}': no parent workflow."
            )
            return None
