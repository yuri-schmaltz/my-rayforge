...
# Define the border size as a constant
BORDER_SIZE = 2
...
def prepare_surface_for_tracing(surface):
    ...
    # Add a white border to make room for a path that may
    # extend slightly beyond the image bounds
    border_color = [255, 255, 255, 255] if channels == 4 else [255, 255, 255]
    ...
    # Replace transparent pixels with white (on the padded image)
    if channels == 4:
...
def contours2ops(contours, pixels_per_mm, ymax):
    ...
    for contour in contours:
        ...
        if len(contour) > 0:
            ops.move_to(
                contour[0][0][0] / scale_x, ymax - contour[0][0][1] / scale_y
            )
...
class OutlineTracer(OpsProducer):
    ...
    def run(self, machine, laser, surface, pixels_per_mm):
        ...
        # Adjust contours for the border offset
        # Using a direct NumPy operation is efficient
        adjusted_contours = [(contour - BORDER_SIZE) for contour in contours]

        ymax = surface.get_height() / pixels_per_mm[1]
        return contours2ops(adjusted_contours, pixels_per_mm, ymax)
