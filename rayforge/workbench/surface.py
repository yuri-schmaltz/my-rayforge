...
from gi.repository import Graphene, Gdk, Gtk, Gio
...
class WorkSurface(Canvas):
    ...
    def _setup_file_drop_target(self):
        """
    ...
    def _on_file_dropped(
        ...
        try:
            ...
            world_x_mm, world_y_mm = self._get_world_coords(x, y)
            logger.info(
            ...
            # Get the main window for import delegation
            main_window = self._get_main_window()
            if not main_window:
                logger.error("Could not get main window for import")
                return False

            # Categorize files by type for batch processing
